...
- **P9.6 Frontend surfacing.** Add `"embedded"` to the TypeScript `ProviderKind` union so it mirrors the Rust enum; confirm zero-priced embedded models fall under the Local group in the `ProviderModelPicker` (via the existing zero-price heuristic); replace the stale "coming soon" LocalRuntimes empty-state with an embedded-engine settings surface to import/select/load a local model.
