...
### 6.5 Feeding the policy real signal (candidate pool, capabilities, cost, quality, and unavailability)

The scoring in Section 6.2 is only as good as the `AvailableModel` inputs it ranks. The policy engine itself is unchanged; the following enrichments make its inputs truthful so `Automatic` actually differentiates models.

1. **Candidate-pool filtering to chat-capable models**. Enumeration excludes model families that cannot answer a chat turn (embeddings, text-to-speech and native-audio, transcription, image generation, video, music, robotics, computer-use, AQA, and deep-research variants). The reliable primary signal is the provider's own capability metadata: for Gemini, `list_models` reads each entry's `supportedGenerationMethods` and keeps only models that advertise `generateContent` (or `streamGenerateContent`). When a provider omits that list for an entry, a documented name-family backstop drops the obvious non-chat families by id substring, and any unrecognized name is kept so a genuinely new chat model is never dropped. The result is that non-chat model families never enter the routing candidate pool or the model picker.

2. **Real per-model capability detection**. `Capabilities` are derived per model from provider metadata rather than a uniform stamp. For Gemini, `max_context` comes from the entry's `inputTokenLimit`, and `vision` is set only for documented multimodal families (the 1.5/2.x/3.x pro and flash lines) rather than claimed for every model; `streaming`/`tools`/`json_mode` reflect what the kept `generateContent` models actually support. Where a provider exposes no metadata, adapters fall back to sane per-family defaults and never over-claim. Two models from the same provider can therefore carry different capabilities and context windows, which is what lets `quality-for-complexity` separate them.

3. **Cost model (bundled representative defaults, then user overlay)**. Consistent with the Section 6.2 pricing paragraph, the pricing table is seeded from bundled representative per-`ProviderKind` defaults and then overlaid by the user-editable per-kind/per-model rates in the versioned app config (user rates win). This fixes the prior behavior where a fresh install priced everything at zero and mislabeled paid cloud models as "free". Genuinely local kinds (LM Studio, Ollama, loopback generic-OpenAI, and the embedded engine) stay at zero cost and read as "free"; cloud kinds are nonzero by default (for example, Gemini is seeded at a representative rate on the order of 1.25 in / 5.0 out per million tokens). These bundled numbers are approximate editable defaults, not an authoritative vendor feed, and carry the same last-edited/stale-pricing caveat as Section 6.2. The picker's local-vs-cloud grouping keys off this zero-price signal, so nonzero-priced cloud models now group under Cloud and zero-price local models stay under Local.

4. **Per-model quality tier**. `AvailableModel` carries a `quality` value in the range 0.0..=1.0 (JSON key `quality`), resolved from a documented, generic, user-overridable per-family/per-kind tier map (pro/opus-tier high, flash/sonnet-tier mid, flash-lite/mini/nano-tier lean, local a middle value). `auto_default`'s `quality_for_complexity` blends this explicit tier with the existing capability/context proxy rather than replacing it, and the complexity weighting still applies (High complexity weights quality more). This is what makes `Automatic` and `Prefer-Quality` pick a stronger model for hard tasks while low-complexity turns still prefer an adequate cheaper or local model, instead of all models scoring identically.

5. **404 / unavailable-model handling**. Because per-model capability, cost, and quality signals now drive selection, `Automatic` ranks toward current models and stops favoring a stale one purely by name. When a routed model is unavailable at request time (for example a Gemini `404` for a deprecated id), the provider error propagates verbatim to a visible `MessageError` and is persisted on an error-status message, so the model's replacement suggestion (for example "use `models/gemini-3.1-pro-preview`") is surfaced to the user rather than swallowed. Gemini's `/models` listing exposes no reliable deprecation marker to filter on at enumeration time, so no speculative filter is applied there; ranking plus the verbatim error is the accepted behavior. An automatic fallback hop that retries the turn with the next-best surviving candidate was deferred: the current `run_turn` is a single routing decision followed by a bounded tool loop, and a clean one-hop retry would require reworking that decision flow, so it is left as a future enhancement.
